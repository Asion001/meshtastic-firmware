...
The Cardputer ADV build includes a dedicated offline map screen opened by pressing uppercase `M` from any normal screen. Lowercase `m` retains its global Messages shortcut, including while the map is open. The map uses the tile format from [lunarc3/CardputerGPSMap](https://github.com/lunarc3/CardputerGPSMap) and reads JPEG tiles arranged as `{zoom}/{x}/{y}.jpg` under `/gpsmap` on the SD card. Generate the tile tree with that project's map converter, then copy the resulting `gpsmap` directory to the card. A bounded locator also recognizes an extra wrapper directory such as `/gpsmap/gpsmap/{zoom}/{x}/{y}.jpg`. Without a GPS or usable saved map position, the screen centers itself on the first available tile and omits the user-position marker.
